...
const CANDIDATE_LIMIT = Math.max(CATALOG_PAGE_SIZE, Math.min(140, Number(process.env.CATALOG_CANDIDATE_LIMIT || 100)));
const LATEST_CANDIDATE_LIMIT = Math.max(80, Math.min(160, Number(process.env.LATEST_CANDIDATE_LIMIT || 120)));
const LOCAL_CANDIDATE_LIMIT = Math.max(30, Math.min(100, Number(process.env.LOCAL_CANDIDATE_LIMIT || 60)));
...
function strictDubLanguage(file) {
  ...
  return { cz: key === 'CZ' || key === 'CZ-SK', sk: key === 'SK' || key === 'CZ-SK' };
}
...
async function mapTmdbItems(type, items, limit = CANDIDATE_LIMIT) {
  ...
  for (const item of items || []) {
    ...
    seen.add(uniqueKey); unique.push(item);
  ...
  const mapped = await mapWithConcurrency(unique, 8, async item => {
    try {
      ...
      return {
        ...
        description: item.overview || '', releaseInfo: releaseDate ? releaseDate.slice(0, 4) : '',
        year: releaseDate ? Number(releaseDate.slice(0, 4)) : undefined, genres: [], _releaseDate: releaseDate,
        _nativeLocale: item._nativeLocale || undefined, _tmdbId: item.id
      };
    } catch { return null; }
  });
...
async function tmdbCandidates(type, skip = 0, mode = 'latest') {
  ...
  const pageCount = mode === 'concerts' ? 8 : 8;
  ...
  const mapped = await mapTmdbItems(type, rows, mode === 'latest' ? LATEST_CANDIDATE_LIMIT : CANDIDATE_LIMIT);
...
async function tmdbLocalCandidates(type, skip = 0) {
  ...
  const payloads = await mapWithConcurrency(requests, 4, req => {
    const params = {
      language: req.nativeLocale === 'cz' ? 'cs-CZ' : 'sk-SK', page: req.page, include_adult: 'false',
      sort_by: isSeries ? 'first_air_date.desc' : 'primary_release_date.desc', with_original_language: req.language, with_origin_country: req.country
    };
    return fetchJson(tmdbUrl(path, params), { headers: tmdbHeaders() })
...
async function cinemetaCandidates(type, skip = 0, pages = 1) {
  ...
  const payloads = await mapWithConcurrency(offsets, 2, offset => fetchJson(`https://v3-cinemeta.strem.io/catalog/${type}/top.json?skip=${offset}`, { headers: { 'User-Agent': `FastShare-Webshare/${VERSION}` } }));
...
function mergeCandidates(local, global, limit = 120) {
  ...
  const push = item => { const id = String(item?.id || ''); if (!id || seen.has(id)) return; seen.add(id); out.push(item); };
  const max = Math.max(local?.length || 0, global?.length || 0);
  for (let i = 0; i < max && out.length < limit; i++) { if (local?.[i]) push(local[i]); if (global?.[i]) push(global[i]); }
  return out;
}

async function catalogCandidates(type, def, skip) {
  if (def.source === 'latest') {
    const [tmdb, local] = await Promise.all([tmdbCandidates(type, skip, 'latest'), def.requireDub === false ? Promise.resolve([]) : tmdbLocalCandidates(type, skip)]);
    if (tmdb.length || local.length) return mergeCandidates(local, tmdb, LATEST_CANDIDATE_LIMIT);
    const fallback = await cinemetaCandidates(type, skip, 3);
  ...
  return mergeCandidates(local, global, Math.max(100, CANDIDATE_LIMIT));
}

function providerCreds(config) { return { fastshare: config?.fastshare || {}, webshare: config?.webshare || {} }; }
...
async function searchOneProvider(terms, provider, auth, maxTerms = 1) {
  ...
  const responses = await mapWithConcurrency(selected, Math.min(SEARCH_CONCURRENCY, 2), term => provider === 'fastshare' ? searchFastshare(term, auth.hash) : searchWebshare(term, auth.token));
  return responses.flatMap(result => Array.isArray(result?.files) ? result.files : []);
}

function strictSeriesTitleEvidence(file, meta) {
  const raw = String(file?.name || ''), name = normalize(raw);
  if (!name) return false;
  const aliases = getTitleAliases(meta);
  const titleOk = aliases.some(alias => {
    const n = normalize(alias); if (!n) return false; if (name.includes(n)) return true;
...
async function availabilityForMeta(meta, type, def, auth, base = null) {
  ...
  const maxTerms = def.source === 'concerts' ? 1 : def.source === 'latest' ? 3 : 2;
  const [fastFiles, webFiles] = await Promise.all([
    searchOneProvider(terms, 'fastshare', auth.fastshare, maxTerms), searchOneProvider(terms, 'webshare', auth.webshare, maxTerms)
  ]);
  const files = [...fastFiles.map(file => ({ ...file, provider: 'fastshare' })), ...webFiles.map(file => ({ ...file, provider: 'webshare' }))];
...
function metaToCatalogItem(base, match, type, def, meta) {
  ...
  if (type === 'movie') behaviorHints.defaultVideoId = base.id; else delete behaviorHints.defaultVideoId;
  const item = {
    id: base.id, type, name: preferredLocalizedTitle(meta, base.name), poster: base.poster, background: base.background, logo: base.logo,
    description: base.description, releaseInfo: base.releaseInfo, year: base.year, imdbRating: base.imdbRating, genres: base.genres,
    director: base.director, cast: base.cast, runtime: base.runtime, trailers: base.trailers, links: base.links, behaviorHints
  };
  let prefix;
  if (base?._nativeLocale === 'cz') prefix = `Český originál dostupný cez ${match?.provider === 'fastshare' ? 'FastShare' : 'Webshare'}.`;
  else if (base?._nativeLocale === 'sk') prefix = `Slovenský originál dostupný cez ${match?.provider === 'fastshare' ? 'FastShare' : 'Webshare'}.`;
  else if (def.source === 'concerts') prefix = `Koncert dostupný cez ${match?.provider === 'fastshare' ? 'FastShare' : 'Webshare'}.`;
  else if (def.requireDub === false) prefix = `Dostupné cez ${match?.provider === 'fastshare' ? 'FastShare' : 'Webshare'}.`;
  else prefix = `CZ/SK dabing nájdený cez ${match?.provider === 'fastshare' ? 'FastShare' : 'Webshare'}.`;
...
async function buildCatalog({ type, id, skip = 0, config, configKey = '' }) {
  ...
  const cacheKey = `catalog-v11:${configKey}:${type}:${id}:${normalizedSkip}`;
  ...
  const checked = await mapWithConcurrency(candidates, 5, async base => {
    try {
  ...
  const value = {
    metas, auth: { fastshare: auth.fastshare.ok, webshare: auth.webshare.ok }, generatedAt: new Date().toISOString(),
    source: def.source || 'cinemeta+tmdb-local', tmdbEnabled: tmdbEnabled(), candidates: candidates.length,
    localCandidates: candidates.filter(item => item?._nativeLocale).length, cache: 'miss'
  };
...
module.exports = {
  CATALOGS, catalogDef, buildCatalog, hasCzSkAudio, qualityMatches, strictDubLanguage, strictSeriesTitleEvidence,
  preferredLocalizedTitle, nativeLocaleAllowed, tmdbCandidates, tmdbLocalCandidates, catalogCandidates
};
